...
# global constants
harmonics = [2,3/2,4/3,5/4,6/5,5/6,3/4,2/3,1/2]
precision = 0.05
test_folder_path = ""
...
def assess_quality(path_to_wav_file):
    ...
    count=1
    while i < 20:
        num = harmonicness(peaks[0], peaks[i-1])
        if num > 0:
            quality+=num
            count+=1
        i+=1
...
def harmonicness(ratio_tpl_2, ratio_tpl_1):
    ...
    for h in harmonics:
        if within_threshold(ratio_tpl_2[1]/ratio_tpl_1[1], h, precision):
            ...
            is_harmonic = True
    if not is_harmonic:
        num -= 0.5 * (ratio_tpl_2[1]/ratio_tpl_1[1])
    return num
...
def find_best_overlay(path_song1, song1_data, path_song2, song2_data, name_song1, name_song2):
    ...
    if beats1[0] < beats2[0]: 
        for t in range(0, 3):
            ...
            output_path = test_folder_path + '/mashups/' + name_song1 + '_' + name_song2 + '_' + str(t) + '.wav'
        ...
        for t in range(0, 3):
            ...
            output_path = test_folder_path + '/mashups/' + name_song2 + '_' + name_song1 + '_' + str(t) + '.wav'
...
if __name__ == "__main__":
    ...
    if qual:          
        ...
        with open(test_folder_path + '/qualities.txt', 'a') as f:
            ...
            print("Best mashup:" + str(max_mashup) + "(" +str(max_quality) + ")")
            shutil.copy(max_mashup, test_folder_path+'/mashups/best_' + os.path.basename(max_mashup))
